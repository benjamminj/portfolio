...
import type { CompileContext, Root } from "mdast-util-from-markdown/lib";
 ...
 */
async function parseMarkdownToMdast(markdown: string) {
	const tree = fromMarkdown(markdown, {
		extensions: [gfm()],
		mdastExtensions: [gfmFromMarkdown()],
	});

	const highlightedHast = highlightCodeBlocks(tree);
 ...
 */
function highlightCodeBlocks(ast: Root): Root {
 ...
 */
function pruneAstNode(node: Node): Omit<Node, "position"> {
...
function pruneAst(ast: Root) {
	...
	const transformAstNode = (node: Node): TransformedNode => {
		...
		if ("children" in node) {
			const transformedChildren: TransformedNode[] =
				node.children.map(transformAstNode);
			children = transformedChildren;
		}

		let pruned = pruneAstNode(node);
...
export type PrunedHast = Awaited<
	ReturnType<typeof MarkdownService.parseMarkdownToMdast>
>;
