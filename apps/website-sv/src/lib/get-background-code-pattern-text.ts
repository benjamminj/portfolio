export function getBackgroundCodePatternText({
	height,
	width,
	...
	if (height === 0 || width === 0) return { text: "", reps: 0 };
	...
	// Add 1 to the final repetitions to account for any rounding errors
	// at certain screen sizes.
	const reps = Math.ceil(totalCharacters / text.length) + 1;

	return { text: text.repeat(reps), reps };
}
